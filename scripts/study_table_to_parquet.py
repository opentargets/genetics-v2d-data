...
def main():
    ...
    missing_ids = from_sumstats_series[~from_sumstats_series.isin(merged['study_id'])]
    ...
    manual_additions = manual_additions.loc[manual_additions['study_id'].isin(missing_ids)]
    ...
    num_loci = (
        top_loci.groupby('study_id').size().reset_index(name='counts').rename(columns={'counts': 'num_assoc_loci'})
    )
...
def parse_args():
    ...
    parser.add_argument(
        '--sumstat_studies', metavar="<str>", help=("List of studies with sumstats"), type=str, required=True
    )
